...
def mensaje_usuario(texto):
    print(Fore.GREEN + Style.BRIGHT + f"🧍 Tú: {texto}".rjust(80) + Style.RESET_ALL)

def mensaje_bot(texto):
    print(Fore.CYAN + Style.BRIGHT + f"🤖 Bot: {texto}" + Style.RESET_ALL)
...
def buscar_productos(id_carrito):
    mensaje_bot("🔍 ¿Qué producto estás buscando? Escribe el nombre del producto.")
    nombre_producto = input("🧍 Tú: ").strip().lower()
    mensaje_usuario(nombre_producto)

    conn = conectar_db()
    cursor = conn.cursor()
    cursor.execute("SELECT id_producto, nombre, precio FROM productos WHERE LOWER(nombre) LIKE %s", (f"%{nombre_producto}%",))
    productos = cursor.fetchall()
    conn.close()

    if not productos:
        mensaje_bot("❌ No encontré productos con ese nombre.")
        return

    mensaje_bot("🔍 Encontré estos productos:")
    for prod in productos:
        mensaje_bot(f"💡 {prod[0]} - {prod[1]} - ${int(prod[2]):,}".replace(",", "."))

    mensaje_bot("Escribe el ID del producto que te interesa agregar al carrito:")
    seleccion = input("🧍 Tú: ").strip()
    mensaje_usuario(seleccion)

    try:
        id_producto = int(seleccion)
        mensaje_bot("¿Cuántas unidades deseas agregar al carrito?")
        cantidad = input("🧍 Tú: ")
...
3️⃣ Preguntar precio de un producto
4️⃣ Ver carrito
...
mensaje_bot("---            BIENVENIDO                ---")
mensaje_bot("--- Ferretería Ferrelectrik Chatbot 🛠️ ---")
mensaje_bot("📱 Ingresa tu número de teléfono:")
telefono = input("🧍 Tú: ").strip()
mensaje_usuario(telefono)
manejar_conversacion(telefono)
